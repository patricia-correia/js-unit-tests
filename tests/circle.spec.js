...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  it('Verifica seqq ao receber um raio, a função `circle` retorna um objeto contendo os valores esperados', () => {
  ...
  it('Verifica se o objeto retornado possui 3 propriedades.', () =>{
    expect(circle(1)).toHaveProperty('radius','area', 'circunference');
  });
    // Teste se a função, quando não recebe nenhum parâmetro, retorna undefined.
  it('Verificar se a função, quando não recebe nenhum parâmetro, retorna undefined', () =>{  
  ...
  it('Verifica se a função retorna, em um objeto, os dados corretos de um círculo de raio 3', () =>{
    expect(circle(3)).toBe({radius:3, area:28.26, circumference:18.84})
  });
});
